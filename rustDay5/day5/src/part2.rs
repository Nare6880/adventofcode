...
fn get_seeds_arr(seed_line: &String) -> Vec<Vec<i64>>{
    ...
    for seed in seeds.get(1).unwrap().split(" "){
        if seed != ""{
            if index%2 == 0{
                result.push([seed.parse::<i64>().unwrap()].to_vec());
            }
            else{
                let last_index = result.len()-1;
                let start_of_last = result[last_index][0];
                result[last_index].push(start_of_last + seed.parse::<i64>().unwrap()-1);
            }
            index+=1;
        }
...
pub fn part2(){
    ...
    let seeds:Vec<Vec<i64>> = get_seeds_arr(&lines_arr.remove(0));
    let mappings:Vec<Vec<Vec<i64>>> = get_mappings(lines_arr);
    let bar = ProgressBar::new((seeds.len()+1) as u64);
    bar.set_style(ProgressStyle::with_template("[{elapsed_precise}] {bar:40.cyan/blue} {pos:>7}/{len:7} {msg}")
    .unwrap()
    .progress_chars("##-"));
    bar.inc(1);
    let min = seeds.par_iter()
                        .map(|seed| {
                                let seed_min = (seed[0]..seed[1]).into_par_iter()
                                    .map(|x| get_mapped_seed(x, &mappings))
                                    .min().unwrap();
                                bar.inc(1);
                                return seed_min;
                            }).min().unwrap();
